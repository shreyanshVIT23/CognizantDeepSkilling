...
describe('Course Selectors', () => {
  const mockState: { course: CourseState } = {
    course: {
      courses: [
        { id: 1, name: 'Angular', code: 'ANG101', credits: 3, gradeStatus: 'pending', enrolled: false }
      ],
      loading: true,
      error: 'Some error'
    }
  ...
  it('should select course loading flag', () => {
    const result = selectCourseLoading(mockState);
    expect(result).toBe(true);
  });

  it('should select course error', () => {
    const result = selectCourseError(mockState);
